...
- 端口可更换:接收端在 GUI 里即可改端口(默认 2022;标准用户被防火墙拦截时,可改用本机已放行的端口如 8080),发送端自动发现会带上正确端口
- 多文件夹同时传输:传输中点"再传一个文件夹"即可并行再发,进度页每任务一张卡片;默认在接收端建同名子文件夹,内容不混淆
- 接收端免重启换批:一批传完点"完成接收,继续收下一个"即可直接收下一个文件夹;接收端还能实时看到"已接收 N 个文件"
- 断点记录按设备独立保存:首页列出每台设备的未完成任务,可分别继续,互不覆盖;两台设备可同时续传。接收端可沿用上次配对码,隔天续传无需重新配对
- 失败原因看得见:连不上接收端 / 配对码不对 / 文件被占用,提示各不相同;接收端换了配对码时,任务卡片上直接输入新码即可续传,无需重走向导
- 扫过的文件夹不重扫:再次选择同一文件夹直接使用上次扫描结果(可手动重扫),百 GB 大目录零等待进入下一步
